...
Run:
    python3 peano_k3_pipelining_bug/test_L2bn_k3s1_16ic_16oc_160.py            # HANGS
    python3 peano_k3_pipelining_bug/test_L2bn_k3s1_16ic_16oc_160.py --scalar   # PASSES
"""
...
def main():
    ...
    print(f"L2bn: k3s1 IC={IC} OC={OC} {H}x{W} -> {H}x{W}  [{mode}]")
    ...
    xclbin_art, insts_art = op.get_artifacts(prefix=f"L2bn_{tag}_")
    extra_flags = ["-DINT8_ACT", "-ffunction-sections", "-fdata-sections"]
    mac_obj = KernelObjectArtifact.new(
        f"conv2dk3_i8_silu_L2bn_{tag}.o",
        depends=[SourceArtifact.new(BUG_DIR / kernel_cc)],
        extra_flags=extra_flags,
    )
    silu_obj = KernelObjectArtifact.new(
        f"silu_postproc_i8_L2bn_{tag}.o",
        depends=[SourceArtifact.new(BUG_DIR / "silu_postproc_i8.cc")],
        extra_flags=extra_flags,
    )
    kernel_dep = LinkedKernelObjectArtifact.new(
        f"conv2dk3_i8_silu_L2bn_{tag}_linked.o", depends=[mac_obj, silu_obj],
    )
    mlir_dep = xclbin_art.depends[0]
    xclbin_art = XclbinArtifact.new(f"L2bn_{tag}.xclbin", depends=[mlir_dep, kernel_dep])
    insts_art = InstsBinArtifact.new(f"L2bn_{tag}.bin", depends=[mlir_dep])
