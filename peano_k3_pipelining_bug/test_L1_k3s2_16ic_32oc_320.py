...
IC=16 means 2 ic_groups in the MMUL inner loop. Still fast enough to
trigger the Peano pipelining bug with split-compiled SiLU.
...
def main():
    ...
    print(f"L1: k3s2 IC={IC} OC={OC} {H}x{W} -> {H//2}x{W//2}")
    ...
    xclbin_art, insts_art = op.get_artifacts(prefix="L1_bug_")
    ...
    kernel_dep = LinkedKernelObjectArtifact.new(
        "conv2dk3_i8_silu_L1_bug.o", depends=[mac_obj, silu_obj],
    )
    mlir_dep = xclbin_art.depends[0]
    xclbin_art = XclbinArtifact.new("L1_bug.xclbin", depends=[mlir_dep, kernel_dep])
    insts_art = InstsBinArtifact.new("L1_bug.bin", depends=[mlir_dep])
