...
from operators import AIERMSNorm, AIEGEMM, AIEGEMV, AIEElementwiseAdd
...
class AIELlamaOperators:
    ...
    def __init__(self, config, prompt_len):
        ...
        self.decode.attn_repeat_interleave = (
            AIERepeat(
...
class AIEPrefillBuffers:
    def __init__(self, prompt_len, emb_dim, hidden_dim, n_heads, n_kv_groups, head_dim):
        ...
        num_pipelines = 8
        S_pad = ((prompt_len + 63 * num_pipelines) // (64 * num_pipelines)) * (
            64 * num_pipelines
        )
        self.mha_q = AIEBuffer(
            shape=(n_heads * S_pad * head_dim,), dtype=ml_dtypes.bfloat16
        )
        self.mha_k = AIEBuffer(
            shape=(n_heads * S_pad * head_dim,), dtype=ml_dtypes.bfloat16
        )
        self.mha_v = AIEBuffer(
            shape=(n_heads * S_pad * head_dim,), dtype=ml_dtypes.bfloat16
        )
        self.mha_o = AIEBuffer(
            shape=(n_heads * S_pad * head_dim,), dtype=ml_dtypes.bfloat16
        )
...
class AIEDecodeBuffers:
    def __init__(
        self, emb_dim, hidden_dim, n_heads, n_kv_groups, head_dim, max_context_len
    ):
        ...
        # Attention buffers: queries and keys serve as both projection output and RoPE input/output
        self.queries = AIEBuffer(
            shape=(1 * n_heads, head_dim), dtype=ml_dtypes.bfloat16
        )
        self.keys = AIEBuffer(
            shape=(1 * n_kv_groups, head_dim), dtype=ml_dtypes.bfloat16
        )
        self.values = AIEBuffer(
            shape=(1, n_kv_groups * head_dim), dtype=ml_dtypes.bfloat16
        )
        ...
        self.attn_scores_keys = AIEBuffer(
            shape=(n_heads, max_context_len, head_dim), dtype=ml_dtypes.bfloat16
        )
        self.attn_scores_values = AIEBuffer(
            shape=(n_heads, max_context_len, head_dim), dtype=ml_dtypes.bfloat16
        )
        self.attn_scores_values_transposed = AIEBuffer(
            shape=(n_heads, head_dim, max_context_len), dtype=ml_dtypes.bfloat16
        )
        ...
        self.attn_context = AIEBuffer(
            shape=(n_heads, head_dim), dtype=ml_dtypes.bfloat16
        )
        self.attn_scores = AIEBuffer(
            shape=(n_heads, max_context_len), dtype=ml_dtypes.bfloat16
        )
    ...
    def __init__(self, config, prompt_len):
        ...
        for layer_idx in range(config.n_layers):
            ...
            self.W_attn_query_decode.append(
                AIEBuffer.from_torch(
                    config.weights[f"model.layers.{layer_idx}.self_attn.q_proj.weight"]
                ).to("npu")
            )
            ...
            self.W_attn_key_decode.append(
                AIEBuffer.from_torch(
                    config.weights[f"model.layers.{layer_idx}.self_attn.k_proj.weight"]
                ).to("npu")
            )
            ...
            self.W_ffn_gate_decode.append(
                AIEBuffer.from_torch(
                    config.weights[f"model.layers.{layer_idx}.mlp.gate_proj.weight"]
                ).to("npu")
            )
            self.W_ffn_up_decode.append(
                AIEBuffer.from_torch(
                    config.weights[f"model.layers.{layer_idx}.mlp.up_proj.weight"]
                ).to("npu")
            )
            self.W_ffn_down_decode.append(
                AIEBuffer.from_torch(
                    config.weights[f"model.layers.{layer_idx}.mlp.down_proj.weight"]
                ).to("npu")
            )
...
def grouped_query_attention_forward_prefill(
    ...
    S_pad = ((target_len + 63 * num_pipelines) // (64 * num_pipelines)) * (
        64 * num_pipelines
    )
...
def grouped_query_attention_forward_decode(
    ...
    aie_ops.decode.gemv_attn_query(
        aie_buffers.W_attn_query_decode[layer_idx],
        aie_buffers.decode.x_norm,
        aie_buffers.decode.queries,
    )
    aie_ops.decode.gemv_attn_key(
        aie_buffers.W_attn_key_decode[layer_idx],
        aie_buffers.decode.x_norm,
        aie_buffers.decode.keys,
    )
    aie_ops.decode.gemv_attn_value(
        aie_buffers.W_attn_value_decode[layer_idx],
        aie_buffers.decode.x_norm,
        aie_buffers.decode.values,
    )

    # Step 2: Apply RoPE - use same buffers for input and output
    aie_ops.decode.rope_queries(
        aie_buffers.decode.queries,
        aie_buffers.decode.rope_angles,
        aie_buffers.decode.queries,
    )
    aie_ops.decode.rope_keys(
        aie_buffers.decode.keys, aie_buffers.decode.rope_angles, aie_buffers.decode.keys
    )
    ...
    aie_ops.decode.strided_copy_cache(
        aie_buffers.decode.keys, aie_buffers.keys_cache[layer_idx]
    )
    aie_ops.decode.strided_copy_cache(
        aie_buffers.decode.values, aie_buffers.values_cache[layer_idx]
    )

    # Step 4: Repeat keys and values for grouped attention using AIERepeat on NPU
    group_size = config.n_heads // config.n_kv_groups
    aie_ops.decode.attn_repeat_interleave(
        aie_buffers.keys_cache[layer_idx], aie_buffers.decode.attn_scores_keys
    )
    aie_ops.decode.attn_repeat_interleave(
        aie_buffers.values_cache[layer_idx], aie_buffers.decode.attn_scores_values
    )

    # Step 5: Compute attention scores
    # Copy repeated keys from keys_repeated buffer to attn_scores_keys for GEMV
    aie_ops.decode.gemv_attn_scores(
        aie_buffers.decode.attn_scores_keys,
        aie_buffers.decode.queries,
        aie_buffers.decode.attn_scores,
    )
    # Apply scaling on NPU
    aie_ops.decode.attn_scale(
        aie_buffers.decode.attn_scores,
        aie_buffers.decode.attn_scale_factor,
        aie_buffers.decode.attn_scores,
    )

    # Step 7: Softmax on NPU (patched once at beginning of decode pass)
    aie_ops.decode.softmax(
        aie_buffers.decode.attn_scores, aie_buffers.decode.attn_weights
    )
    ...
    aie_ops.decode.gemv_attn_context(
        aie_buffers.decode.attn_scores_values_transposed,
        aie_buffers.decode.attn_weights,
        aie_buffers.decode.attn_context,
    )

    # Read context from NPU
    aie_buffers.decode.attn_context.to("cpu")
    context = aie_buffers.decode.attn_context.view_as_torch().unsqueeze(
        1
    )  # (n_heads, 1, head_dim)
...
def swiglu_ffn_forward_decode(layer_idx):
    # Step 1: Gate projection
    aie_ops.decode.gemv_ffn_up_gate(
        aie_buffers.W_ffn_gate_decode[layer_idx],
        aie_buffers.decode.x_norm,
        aie_buffers.decode.ffn_gate,
    )

    # Step 2: Up projection
    aie_ops.decode.gemv_ffn_up_gate(
        aie_buffers.W_ffn_up_decode[layer_idx],
        aie_buffers.decode.x_norm,
        aie_buffers.decode.ffn_up,
    )
    ...
    # Step 4: Element-wise multiplication
    aie_ops.decode.eltwise_mul_ffn(
        aie_buffers.decode.ffn_gate,
        aie_buffers.decode.ffn_up,
        aie_buffers.decode.ffn_hidden,
    )

    # Step 5: Down projection
    aie_ops.decode.gemv_ffn_down(
        aie_buffers.W_ffn_down_decode[layer_idx],
        aie_buffers.decode.ffn_hidden,
...
def transformer_block_forward_decode(
    config, seq_len, num_preceding_tokens, layer_idx, attn_mask
):
    # Step 1: RMS normalization
    aie_ops.decode.rms_norm(
        aie_buffers.decode.x, aie_buffers.W_norm1[layer_idx], aie_buffers.decode.x_norm
    )
    ...
    # Step 3: Residual
    aie_buffers.decode.attn_output.view_as_torch().unsqueeze(0)[
        0, :seq_len, :
    ] = attn_output
    aie_ops.decode.residual_add(
        aie_buffers.decode.x, aie_buffers.decode.attn_output, aie_buffers.decode.x
    )
    ...
    aie_ops.decode.rms_norm(
        aie_buffers.decode.x, aie_buffers.W_norm2[layer_idx], aie_buffers.decode.x_norm
    )
    aie_buffers.decode.x_norm.to("cpu")
    x_norm = aie_buffers.decode.x_norm.view_as_torch().unsqueeze(0)[:, :seq_len, :]
    ...
    # Step 6: Residual
    aie_ops.decode.residual_add(
        aie_buffers.decode.x, aie_buffers.decode.ffn_output, aie_buffers.decode.x
    )
...
def llama_forward_pass_decode(
    ...
    # Step 4: Final normalization
    aie_ops.decode.rms_norm(
        aie_buffers.decode.x, aie_buffers.W_final_norm, aie_buffers.decode.x
    )

    # Step 5: Output projection
    aie_ops.decode.gemv_out_head(
        aie_buffers.W_out_head, aie_buffers.decode.x, aie_buffers.decode.logits
    )
