...
def swiglu_ffn_forward(x, fc1_weight, fc2_weight, fc3_weight):
    ...
    gate_activated = torch.nn.functional.silu(gate)  # (batch, seq_len, swiglu_hidden_dim)
...
def llama_forward_pass(
    ...
    Args:
        ...
        rope_angles: Precomputed RoPE angles
        input_ids: (batch, seq_len) token index/indices
        attn_keys_caches: Previously computed keys for past tokens, one for each transformer layer
        attn_values_caches: Previously computed values for past tokens, one for each transformer layer
    ...
    for layer_idx in range(config.n_layers):
        x, attn_keys_caches[layer_idx], attn_values_caches[layer_idx] = transformer_block_forward(
            x,
            attn_keys_caches[layer_idx],
            attn_values_caches[layer_idx],
            config.n_heads,
            config.n_kv_groups,
            W_norm1=weights[f'model.layers.{layer_idx}.input_layernorm.weight'],
            W_attn_query=weights[f'model.layers.{layer_idx}.self_attn.q_proj.weight'],
            W_attn_key=weights[f'model.layers.{layer_idx}.self_attn.k_proj.weight'],
            W_attn_value=weights[f'model.layers.{layer_idx}.self_attn.v_proj.weight'],
            W_attn_out=weights[f'model.layers.{layer_idx}.self_attn.o_proj.weight'],
            W_ffn_fc1=weights[f'model.layers.{layer_idx}.mlp.gate_proj.weight'],
            W_ffn_fc2=weights[f'model.layers.{layer_idx}.mlp.up_proj.weight'],
            W_ffn_fc3=weights[f'model.layers.{layer_idx}.mlp.down_proj.weight'],
            W_norm2=weights[f'model.layers.{layer_idx}.post_attention_layernorm.weight'],
    ...
    return logits, attn_keys_caches, attn_values_caches
