...
value_cache_1 = AIEBuffer((num_heads, max_prompt_len, head_dim))
value_1 = AIEBuffer((prompt_len, num_heads, head_dim))
value_cache_1.view_as_torch()[:, :cached_prompt_len, :] = torch.randn(
    num_heads, cached_prompt_len, head_dim
)
value_1.view_as_torch()[:prompt_len, :, :] = torch.randn(
...
value_cache.view_as_torch()[:, :cached_prompt_len, :] = torch.randn(
    num_heads, cached_prompt_len, head_dim
)
...
out_ref = torch.cat(
    [value_cache.view_as_torch()[:, :cached_prompt_len, :], value_transposed], dim=1
)
t_cpu = time.perf_counter() - t_cpu_start

transpose_concat(value_1, value_cache_1)
value_cache.to("npu")
value.to("npu")
...
if len(mismatches[0]) > 0:
    for i in range(min(10, len(mismatches[0]))):
        ...
        print(
            f"Mismatch at head={h}, seq={s}, dim={d}: ref={out_ref[h,s,d]}, aie={aie_out[h,s,d]}, diff={diff[h,s,d]}"
        )
