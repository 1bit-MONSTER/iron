...
def fuse_mlir(artifact):
    ...
    for op_name, mlir_artifact in artifact.operator_mlir_map.items():
        ...
        device_ops = [op for op in mlir_module.body.operations if isinstance(op, aie.DeviceOp)]
        assert len(device_ops) == 1, f"Expected exactly one device operation in MLIR artifact for operator '{op_name}'"
                ...
                for op_name, *buffer_names in artifact.runlist:
                    ...
                    with ir.InsertionPoint(configure_body):
                        ...
                        for idx, buf_name in enumerate(buffer_names):
                            ...
                            assert expected_size == size_elements, f"Size mismatch for buffer '{buf_name}': MLIR runtime sequence expected {expected_size}, Python fused operator provided {size_elements}"
                            strides = []
                            stride = 1
