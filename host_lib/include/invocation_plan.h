...
struct KernelInfo {
    ...
    std::string insts; // Path to instruction sequence to run for invocations of this kernel; if you need different
                       // insts.bins for the same kernel, create multiple KernelInfos with different names
    std::string xclbin_name; // Name as defined in the xclbin (aiecc.py --xclbin-kernel-name=XX); can be omitted if the
                             // same as 'name'
};

struct KernelBufferInfo {
    enum Direction { IN = 0b01, OUT = 0b10, INOUT = 0b11 };
    std::string name;
    size_t size;
...
struct InvocationPlan {
    ...
    static InvocationPlan
    fromInfo(const InvocationPlanInfo &info, xrt::device &device, xrt::xclbin &xclbin, xrt::hw_context &context);
  ...
  private:
    ...
    void setupBuffers(const std::vector<KernelBufferInfo> &buffers,
                      const std::vector<KernelInvocationInfo> &invocations);
...
InvocationPlan InvocationPlan::fromInfo(const InvocationPlanInfo &info,
                                        xrt::device &device,
                                        xrt::xclbin &xclbin,
                                        xrt::hw_context &context)
{
...
void InvocationPlan::setupKernels(const std::vector<KernelInfo> &kernel_infos)
{
    for (const KernelInfo &info : kernel_infos) {
        ...
        new_kernel.insts_bo = xrt::bo(device,
                                      new_kernel.insts.size() * sizeof(new_kernel.insts[0]),
                                      XCL_BO_FLAGS_CACHEABLE,
                                      new_kernel.kernel.group_id(1));
...
void InvocationPlan::setupBuffers(const std::vector<KernelBufferInfo> &buffer_infos,
                                  const std::vector<KernelInvocationInfo> &invocation_infos)
{
    ...
    for (const KernelBufferInfo &buffer_info : buffer_infos) {
        ...
        int group_id =
            first_user_kernel.kernel.group_id(first_user.second + 3); // +3 to account for insts buffer and opcode
        for (auto &[user_kernel_name, arg_index] : buffer_users[buffer_name]) {
            ...
            if (user_group_id != group_id) {
                std::stringstream ss;
                ss << "Error: buffer " << buffer_name << " used by multiple kernels with different group IDs"
                   << std::endl;
        ...
        new_buffer.bo = xrt::bo(device, buffer_info.size * sizeof(DTYPE), XCL_BO_FLAGS_HOST_ONLY, group_id);
...
std::vector<std::pair<std::string, unsigned>> InvocationPlan::verifyOutputBuffers(float epsilon, float abs_th) const
{
    ...
    for (const auto &[buffer_name, buffer] : buffers) {
        ...
        for (unsigned i = 0; i < buffer.size; i++) {
            if (!test_utils::nearly_equal(
                    static_cast<float>(buffer.buf[i]), static_cast<float>(buffer.reference[i]), epsilon, abs_th)) {
