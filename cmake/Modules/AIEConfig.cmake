...
function(add_aie_executable
    ...
    set(multiValueArgs HOST_FLAGS PYTHON_FLAGS DEVICE_FLAGS AIE_CORE_KERNELS)
    ...
    add_aie_design(${TARGET_NAME}
        ...
        AIE_CORE_BUNDLES ${ADD_AIE_EXECUTABLE_AIE_CORE_BUNDLES}
...
#     AIE_CORE_BUNDLES (string): The name(s) of bundle(s) from /aie/{aie1,aie2,aie2p} needed by this design.
...
function(add_aie_design TARGET_NAME)
    ...
    set(multiValueArgs PYTHON_FLAGS DEVICE_FLAGS AIE_CORE_KERNELS AIE_CORE_BUNDLES EXTRA_AIECC_FLAGS)
    ...
    if(ADD_AIE_DESIGN_AIE_CORE_KERNELS)
        foreach(CORE_KERNEL IN LISTS ADD_AIE_DESIGN_AIE_CORE_KERNELS)
            ...
            list(APPEND XCLBIN_DEPENDENCIES "${CORE_KERNEL}")
        endforeach()
    endif()    
    ...
    foreach(kernel IN LISTS ADD_AIE_DESIGN_AIE_CORE_KERNELS)
        list(APPEND TMP_KERNEL_COPY_CMDS COMMAND ${CMAKE_COMMAND} -E copy_if_different ${BUILD_DIR}/${kernel} ${AIECC_TMP_BUILD_DIR}/)
    endforeach()
    ...
    add_custom_command(
        ...
        COMMAND ${CMAKE_COMMAND} -E chdir ${AIECC_TMP_BUILD_DIR} 
            aiecc.py --no-compile-host --no-xchesscc --no-xbridge --peano ${IRONCLAD_PEANO_DIR}
                     --aie-generate-xclbin --xclbin-name=${XCLBIN_OUTPUT} --xclbin-kernel-name=${TARGET_NAME}
                     --aie-generate-npu --npu-insts-name=${INSTRUCTIONS_OUTPUT} --progress
                     ${ADD_AIE_DESIGN_EXTRA_AIECC_FLAGS}
                     ${MLIR_OUTPUT}
        DEPENDS ${XCLBIN_DEPENDENCIES}
        COMMENT "Compiling AIE MLIR to xclbin"
        VERBATIM
    )
