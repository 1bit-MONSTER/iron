...
// RMS Norm reference implementation
void rms_norm_reference(const std::bfloat16_t *input, std::bfloat16_t *output, int length)
{
    ...
    // Normalize by RMS
    for (int i = 0; i < length; i++) {
        output[i] = std::bfloat16_t(static_cast<float>(input[i]) / rms);
    }
}
...
int main(int argc, const char *argv[])
{
    ...
    cxxopts::Options options("RMS_NORM Test");
