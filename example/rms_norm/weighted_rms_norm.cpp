...
// Weighted RMS Norm reference implementation
void weighted_rms_norm_reference(const std::bfloat16_t *input,
                                 ...
                                 int length)
{
    ...
    float mean_square = sum_of_squares / length;
    float rms = std::sqrt(mean_square + 1e-5f); // Epsilon

    // Normalize by RMS and multiply by weights
    for (int i = 0; i < length; i++) {
        float normalized = static_cast<float>(input[i]) / rms;
        float weighted = normalized * static_cast<float>(weights[i]);
        output[i] = std::bfloat16_t(weighted);
    }
}
...
int main(int argc, const char *argv[])
{
    ...
    cxxopts::Options options("WEIGHTED_RMS_NORM Test");
    ...
    options.add_options()("help,h",
        ...
        "weight-length,w",
        "the length of the weight vector in std::bfloat16_t",
    ...
    int weight_length = vm["weight-length"].as<int>();
    if ((N % weight_length)) {
        std::cerr << "Length must be a multiple of weight_length (" << weight_length << ")." << std::endl;
        return 1;
    }

    if (verbosity >= 1)
        std::cout << "Weight length: " << weight_length << std::endl;
    ...
    auto bo_inB = xrt::bo(device, weight_length * sizeof(std::bfloat16_t), XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
    ...
    memcpy(bufInB, golden_reference::W.data(), (golden_reference::W.size() * sizeof(std::bfloat16_t)));
