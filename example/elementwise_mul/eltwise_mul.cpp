...
int main(int argc, const char *argv[])
{
    ...
    cxxopts::Options options("ELEMENT_WISE_ADD Test");
    ...
    if ((N % TILE_SIZE)) {
        std::cerr << "Length must be a multiple of " << TILE_SIZE << "." << std::endl;
        return 1;
    }
    ...
    auto bo_inB = xrt::bo(device, N * sizeof(std::bfloat16_t), XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
    ...
    for (int i = 0; i < N; i++) {
        if (!test_utils::nearly_equal(*(bufOut + i), golden_reference::C[i], 0.05, 1e-6)) {
            errors++;
            // Print the first 100 mismatches
            if (errors <= 100) {
                std::cout << "Mismatch at index " << i << ": " << "Expected: " << golden_reference::C[i] << ", "
