...
void check_arg_file_exists(po::variables_map &vm_in, std::string name)
{
    if (!vm_in.count(name)) {
        ...
        if (!test) {
            throw std::runtime_error("The " + name + " file " + vm_in[name].as<std::string>() + " does not exist.\n");
        }
    }
}
...
void add_default_options(po::options_description &desc)
{
    desc.add_options()("help,h", "produce help message")(
        ...
        "N,N", po::value<int>()->default_value(512), "Matrix size N")("iters", po::value<int>()->default_value(1))(
        "warmup", po::value<int>()->default_value(0))("trace_sz,t", po::value<int>()->default_value(0))(
        "trace_file", po::value<std::string>()->default_value("trace.txt"), "where to store trace output")(
        "b_col_maj", po::value<int>()->default_value(0), "Is B matrix in colum-major format?");
}
...
template <typename T> static inline T get_random();

template <> [[maybe_unused]] std::int16_t get_random<std::int16_t>()
{
    return (std::int16_t)rand() % 0x10000;
}

template <> [[maybe_unused]] int8_t get_random<int8_t>()
{
    return (int8_t)rand() % 0x100;
}

template <> [[maybe_unused]] std::bfloat16_t get_random<std::bfloat16_t>()
{
...
template <typename Tin, typename Tout, typename Tacc>
Tout mul_acc([[maybe_unused]] int M,
             int N,
             int K,
             int row,
             int col,
...
template <typename T> static inline float get_abs_tol();
template <typename T> static inline float get_rel_tol();

template <> [[maybe_unused]] float get_abs_tol<std::int16_t>()
{
    return 0.0;
}

template <> [[maybe_unused]] float get_abs_tol<std::int32_t>()
{
    return 0.0;
}

template <> [[maybe_unused]] float get_abs_tol<std::bfloat16_t>()
{
    return 0.5;
}

template <> [[maybe_unused]] float get_abs_tol<float>()
{
    return 0.5;
}

template <> [[maybe_unused]] float get_abs_tol<int8_t>()
{
    return 0;
}

template <> [[maybe_unused]] float get_rel_tol<std::int16_t>()
{
    return 0.0;
}

template <> [[maybe_unused]] float get_rel_tol<std::int32_t>()
{
    return 0.0;
}

template <> [[maybe_unused]] float get_rel_tol<std::bfloat16_t>()
{
    return 0.05;
}

template <> [[maybe_unused]] float get_rel_tol<float>()
{
    return 0.05;
}

template <> [[maybe_unused]] float get_rel_tol<int8_t>()
{
    return 0;
}
...
std::optional<struct error<Tout>> verify_single([[maybe_unused]] std::ostream &os,
                                                int row,
                                                int col,
                                                Tout expected,
                                                Tout actual,
...
template <typename Tout>
void print_error_summary(std::ostream &os, int n_errors, std::vector<struct error<Tout>> &errors, Tout max_rel_error)
{
    for (struct error<Tout> &err : errors) {
        os << "[" << std::setw(5) << err.row << ", " << std::setw(5) << err.col << "] " << std::setw(4)
           << std::setprecision(2) << std::fixed << (float)err.actual << " =!= " << std::setw(4) << std::setprecision(2)
    ...
    for (int row = 0; row < M; row++) {
        for (int col = 0; col < N; col++) {
            std::optional<struct error<Tout>> error =
                verify_single(std::cout, row, col, CRef[row * N + col], C[row * N + col], abs_tol, rel_tol);
            if (error.has_value()) {
                if (n_errors < max_printable_errors) {
...
// Tracing
// --------------------------------------------------------------------------
void write_out_trace(char *traceOutPtr, std::size_t trace_size, std::string path)
{
    std::ofstream fout(path);
    uint32_t *traceOut = (uint32_t *)traceOutPtr;
    for (std::size_t i = 0; i < trace_size / sizeof(traceOut[0]); i++) {
