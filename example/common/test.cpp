...
int main(int argc, const char *argv[])
{
    ...
    const std::string xclbin_path = std::string(XCLBIN_PATH);
    std::vector<uint32_t> instr_v = matmul_common::load_instr_sequence(std::string(INSTRUCTIONS_PATH));
        ...
        if (do_verify) {
            ...
            if (do_verify_stochastic) {
                errors = matmul_common::verify_stochastic<A_DATATYPE, C_DATATYPE, ACC_DATATYPE>(
                    M, N, K, AVec, BVec, CVec, verify_stochastic_n_samples, verbosity, abs_tol, rel_tol, b_col_maj);
            } else {
