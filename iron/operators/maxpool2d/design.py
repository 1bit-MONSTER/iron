...
def my_maxpool2d(
    ...
    """ObjectFIFO design for MaxPool2d on NPU2 with bfloat16.

    Simplified approach: process one output row at a time.
    For each output row, the kernel receives one input strip
    (kernel_size rows worth of data as a single flat buffer)
    and produces one output row.

    This avoids the sliding window acquire(N) pattern which
    has compatibility issues with the dynamic-objFifos pass.
    """
    xfr_dtype = bfloat16
    ...
    # For each output row, we need kernel_size input rows
    # We send them as one contiguous strip
    ...
    # Kernel that processes a strip of kernel_size rows → 1 output row
    # It will internally index into the strip as 5 consecutive rows
    maxpool_kernel = Kernel(
        "maxpool2d_5x5_bf16_strip",
        "maxpool2d_bf16.o",
        [input_strip_ty, output_row_ty, np.int32, np.int32, np.int32],
    )

    # ObjectFIFOs
    # For large channel counts, the strip can exceed L1 double-buffer capacity.
    # L1 budget: strip_size*2 + output_row*2 + stack(1KB) must fit in 64KB.
    # Use depth=1 (single-buffered) for input when strip > 16KB to stay safe.
    strip_bytes = input_strip_size * 2  # bf16 = 2 bytes
    ...
    # A single strip = kernel_size × input_row_elems elements.
    # When input_strip_size > 1023, we must decompose into 4D with all
    # sizes ≤ 1023.
    #
    ...
    #   padded_width * 8 elements (e.g., 12*8=96 for 8ch, or 96 for 128ch)
    #
    # 4D decomposition:
