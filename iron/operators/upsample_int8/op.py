...
class AIEUpsampleInt8(AIEOperatorBase):
    """AIE-accelerated nearest-neighbor 2x upsampling for int8.

    Input:  [1, C, H, W]  in int8
    Output: [1, C, 2H, 2W] in int8
    ...
    def get_artifacts(self, prefix="upsample_int8_"):
    ...
    def set_up_runtime(self):
        ...
        self.add_to_runlist("upsample_int8", "input", "output")
    ...
    def forward(self, x):
        ...
        Returns:
            Output tensor of shape [1, C, 2H, 2W] in int8.
        """
        if x.dtype != torch.int8:
            raise AIEOperatorConstraintError(
                "AIEUpsampleInt8: input must be int8"
            )
        if x.shape[0] != 1:
            raise AIEOperatorConstraintError(
                "AIEUpsampleInt8: batch size must be 1"
            )
        if x.shape[1] != self.channels:
            raise AIEOperatorConstraintError(
                f"AIEUpsampleInt8: expected {self.channels} channels, "
                f"got {x.shape[1]}"
            )
        if x.shape[2] != self.height or x.shape[3] != self.width:
            raise AIEOperatorConstraintError(
                f"AIEUpsampleInt8: expected spatial dims "
